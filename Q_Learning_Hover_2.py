...
# Use a reentrant lock so that nested locking (e.g. inside get_state) is allowed.
...
with sim_lock:
    ...
    data.ctrl[:] = 0.0  # Initialize control inputs

# Thrust levels and increment settings
max_thrust = 5
return_thrust = 3
increment = 0.1  
desired_z = 5.0  # Desired altitude
...
def get_state():
    """Retrieves the current state of the drone in a thread-safe way."""
    with sim_lock:
        state = np.array([
    ...
    # Normalize (adjust these values as needed)
...
n_actions = 4  # Actions: decrease, maintain, increase, max thrust
n_observations = 9  # [x, y, z, vx, vy, vz, roll, pitch, yaw]
...
def thrust_control():
    ...
    while True:
        ...
        # Update control input and step the simulation atomically
        with sim_lock:
        ...
        print(f"Time: {current_flight_time:.2f}, Thrust: {thrust_value:.2f}, "
              f"Position: x={drone_x:.2f}, y={drone_y:.2f}, z={drone_z:.2f}")
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            reward_tensor = torch.tensor([reward], device=device)
                ...
                (current_flight_time > 2.0 and drone_z < 0.01) or
                (current_flight_time > 2.0 and reward <= -20) or
                (current_flight_time > 2.0 and (abs(data.qpos[3]) > 0.5 or abs(data.qpos[4]) > 0.5 or abs(data.qpos[5]) > 0.5))
            )
